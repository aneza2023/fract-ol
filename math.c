...
/*   Updated: 2025/03/26 19:08:22 by ahavrank         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
// double mapp(double new_max, double new_min, double old_max,
// double old_min, int i)
// {
//     double  resized_val;

//     resized_val = (new_max - new_min) * (i - old_min) / (old_max - old_min)
// + new_min;
//     return (resized_val);
// }
...
void	mapping(int x, int y, t_fractal *mandel)
{
	...
	mandel->z->real = 0.0;
	mandel->z->im = 0.0;
	mandel->c->real = (mapp(2, -2, WIDTH, x) / mandel->zoom) + mandel->shift_x;
	mandel->c->im = (mapp(-2, 2, HEIGHT, y) / mandel->zoom) + mandel->shift_y;
	iter = mandel->ite + (int)(mandel->zoom * 0.1);
	while (i < iter)
	{
		mandel->z = add(square_of_nb(mandel->z), mandel->c);
		if (((mandel->z->real * mandel->z->real) + (mandel->z->im
					* mandel->z->im)) > 4)
		{
			color = mapp(BLUE, BABY_BLUE, 50, i + mandel->ite);
